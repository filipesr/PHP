<?php
/**
 * JCollection Googlemapitem element (for usage in parameter panels)
 *
 * @version $Id$
 * @package JCollection
...
class JElementGooglemapitem extends JElement
{
	/**
	* Element name
	*
	...
	*/
	...
	function fetchElement($name, $value, &$node, $control_name)
	{
		...
		if($value) {
			...
			$item->name = JText::_('Find Google map URL');
		}
		...
		$js = "function jSelectItem(id, title, object) {
			...
			function getLocationGoogle() {
				return document.getElementById('googlemapitem_id').value;
			}";
		$doc->addScriptDeclaration($js);

		$link = 'index.php?option='._JC_COMPNAME.'&amp;controller=elements&amp;task=googlemapitem&amp;tmpl=component&amp;object='.$name;
		...
		$html .= '<div class="button2-left"><div class="blank"><span onclick="jSelectItem(\'\',\''.JText::_('Find Google map URL').'\',\''.$name.'\');" title="'.JText::_('Clear selection').'">'.JText::_('Clear').'</span></div></div>'."\n";
