...
class PodcastViewFeed extends JView
{
	function display($tpl = null)
	{
		...
		if($params->get('cache', true)) {
			...
			if($cache->start('feed', 'com_podcast')) { // cache hit
				//echo '<!-- cache hit -->';
				return;
			}
		}
		...
		$xw->startElement('rss');
		$xw->writeAttribute('xmlns:itunes', $namespace);
		...
		$xw->writeAttribute('href', $params->get('itImage', ''));
		$xw->endElement();
	...
	private function addEnclosure(&$xw, &$pcast, $params, $fileURL, $fileFullPath)
	{
		...
		if (count($pieces) < 3) {
			...
			if (!function_exists('mime_content_type')) {
				...
				$pieces[2] = mime_content_type($fileFullPath);
			}	
		}
