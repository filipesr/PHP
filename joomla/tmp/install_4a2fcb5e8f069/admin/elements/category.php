<?php
/**
 * JCollection item element (for usage in parameter panels)
 *
 * @version $Id$
 * @package JCollection
...
class JElementCategory extends JElement
{
	...
	*/
	...
	function fetchElement($name, $value, &$node, $control_name)
	{
		...
		$item =& JTable::getInstance('category', 'Table');
		if ($value) {
			$item->load($value);
		} else {
			$item->name = JText::_('Select a Category');
		}
		...
		$link = 'index.php?option='._JC_COMPNAME.'&amp;controller=elements&amp;task=category&amp;tmpl=component&amp;object='.$name;
		...
		$html .= '<div class="button2-left"><div class="blank"><a href="#" onclick="jSelectItem(\'\',\''.JText::_('Select a category').'\',\''.$name.'\');" title="'.JText::_('Clear selection').'">'.JText::_('Clear').'</a></div></div>'."\n";
