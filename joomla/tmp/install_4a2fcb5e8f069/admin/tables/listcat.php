<?php
/**
 * JCollection List Category Table
 *
 * @version $Id$
 * @package JCollection
...
/**
 * List Category Table class
 *
 */
class TableListcat extends JTable
{
	/**
	 * Primary Key
	 *
	 ...
	 */
	function TableListcat(& $db) {
		parent::__construct('#__'._JC_DB.'_listcat', 'id', $db);
	}
	...
	function check()
	{
		...
		if (trim($this->name) == '') {
			$this->setError(JText::_('The list category must have a name.'));
			return false;
		}

		/** check for existing name */
		$query = 'SELECT id FROM #__'._JC_DB.'_listcat WHERE name = '.$this->_db->Quote($this->name);
		$this->_db->setQuery($query);

		$xid = intval($this->_db->loadResult());
		if ($xid && $xid != intval($this->id)) {
			$this->setError(JText::sprintf('WARNNAMETRYAGAIN', JText::_('JCollection List Category')));
			return false;
		}
	...
	function delete( $cid = null )
	{
		...
		if(count($cid)) {
			$query = "DELETE FROM #__"._JC_DB."_listcat WHERE id IN ( ".implode(",",$cid)." )";
			$this->_db->setQuery($query);
			if(!$this->_db->query()) {
	...
	function archive( $cid = null, $user_id = 0 )
	{
		...
		if (count( $cid ) < 1)
		{
			if ($this->$k) {
				...
				$this->setError("No list categories selected.");
				return false;
			}
		}
