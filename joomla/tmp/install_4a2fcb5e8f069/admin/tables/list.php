...
class TableList extends JTable
{
	...
	function TableList(& $db) {
		parent::__construct('#__'._JC_DB.'_list', 'id', $db);
	}
	...
	function check()
	{
		...
		if (trim($this->name) == '') {
			$this->setError(JText::_('The list must have a name.'));
			return false;
		}

		/** check for existing name */
		$query = 'SELECT id FROM #__'._JC_DB.'_list WHERE name = '.$this->_db->Quote($this->name).' AND catid = '.(int) $this->catid;
		$this->_db->setQuery($query);

		$xid = intval($this->_db->loadResult());
		if ($xid && $xid != intval($this->id)) {
			$this->setError(JText::sprintf('WARNNAMETRYAGAIN', JText::_('JCollection List')));
			return false;
		}
	...
	function delete( $cid = null )
	{
		...
		if(count($cid)) {
			$query = "DELETE FROM #__"._JC_DB."_list WHERE id IN ( ".implode(",",$cid)." )";
			$this->_db->setQuery($query);
			if(!$this->_db->query()) {
	...
	function archive( $cid = null, $user_id = 0 )
	{
		...
		if (count( $cid ) < 1)
		{
			if ($this->$k) {
				...
				$this->setError("No lists selected.");
				return false;
			}
		}
