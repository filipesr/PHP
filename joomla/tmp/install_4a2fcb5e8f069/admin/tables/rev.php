...
class TableRev extends JTable
{
	 ...
	 */
	function TableRev(& $db) {
		parent::__construct('#__'._JC_DB.'_review', 'id', $db);
	}
	...
	function check()
	{
		...
		if ( trim( $this->name ) == '' ) {
			$this->setError( JText::_( 'The review must have a name.' ) );
			return false;
		}

		/** check for existing name */
		$query = 'SELECT id FROM #__'._JC_DB.'_review WHERE name = '.$this->_db->Quote( $this->name ).' AND infoid = '.(int)$this->infoid;
		$this->_db->setQuery($query);

		$xid = intval( $this->_db->loadResult() );
		if ( $xid && $xid != intval( $this->id ) ) {
			$this->setError( JText::sprintf( 'WARNNAMETRYAGAIN', JText::_( 'JCollection Review' ) ) );
			return false;
		}
