...
<table>
	...
	<tr>
		...
		echo JText::_( 'List category:' ).$this->lists['cat'];
		echo "&#160;";
		?></td>
		<td nowrap="nowrap"><?php
...
<table class="adminlist">
	<thead>
		<tr>
			...
			<th><?php echo JHTML::_('grid.sort',  'List category', 'c.id', $this->lists['order_Dir'], $this->lists['order'] ); ?>
			</th>
	...
	foreach( $this->ls as $row )
	{
		...
		$link = JRoute::_( 'index.php?option='.$option.'&controller=list&view=list&task=edit&cid[]='.$row->id );
		?>
	...
	<!-- <tr>  -->
		...
		if (  JTable::isCheckedOut($this->user->get ('id'), $row->checked_out ) ) {
			...
			title="<?php echo JText::_( 'Edit list' );?>::<?php echo $row->name; ?>"><a
...
<input type="hidden" name="view" value="lists" /> <input
	type="hidden" name="option" value="<?php echo $option; ?>" /> <input
	type="hidden" name="task" value="" /> <input type="hidden"
	name="boxchecked" value="0" /> <input type="hidden" name="controller"
	value="lists" /> <input type="hidden" name="filter_order"
