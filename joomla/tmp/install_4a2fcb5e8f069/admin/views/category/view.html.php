...
//		$paramsdefs = JPATH_COMPONENT.DS.'models'.DS.'item.xml';
//		$params = new JParameter( $paramsdata, $paramsdefs );
		...
		$lists['image'] = JHTML::_('list.images',  'img', $cat->img, null, '/images/jcollection/' );

		// build the html select list for ordering
		$query = 'SELECT ordering AS value, name AS text' .
                                ' FROM #__'._JC_DB.'_cat' .
                                ' WHERE parent = ' . (int) $cat->parent .
                                ' AND published >= 0' .
                                ' ORDER BY ordering';
		if(!$isNew) {
		...
		$paramsdata = $cat->params;
		$paramsdefs = JPATH_COMPONENT.DS.'models'.DS.'category.xml';
		$params = new JParameter( $paramsdata, $paramsdefs );
