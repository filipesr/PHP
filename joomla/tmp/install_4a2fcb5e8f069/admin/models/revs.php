<?php
/**
 * JCollection Reviews Model class
 *
 * @version $Id$
 * @package JCollection
 ...
 * @subpackage com_jcollection
 */
class JCollectionModelRevs extends JModel
{
	/**
	 * Reviews data array
	 *
	...
	{
		$model = &JModel::getInstance('rev','JCollectionModel');
	...
	function _buildQuery()
	{
		...
		."\n m.id AS itemid, "
		."\n m.name AS itemname, "
		."\n u.name AS uname, u.username, g.name AS groupname  "
		."\n FROM #__"._JC_DB."_review AS i "
		."\n LEFT JOIN #__"._JC_DB."_info AS f ON f.id = i.infoid "
		."\n LEFT JOIN #__"._JC_DB." AS m ON m.id = f.itemid "
	...
	function _buildContentOrderBy()
	{
		...
		if ($filter_order == 'i.ordering') {
			$orderby = ' ORDER BY m.id, i.ordering '.$filter_order_Dir;
		} else {
			if( trim($filter_order)!='')
			$orderby = ' ORDER BY '.$filter_order.' '.$filter_order_Dir.' , m.id, i.ordering ';
			else
			$orderby = ' ORDER BY m.id, i.ordering ';
		}
