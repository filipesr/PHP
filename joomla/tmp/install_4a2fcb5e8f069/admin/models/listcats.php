<?php
/**
 * JCollection List Categories Model class
 *
 * @version $Id$
 * @package JCollection
 ...
 * @subpackage com_jcollection
 */
class JCollectionModelListcats extends JModel
{
	/**
	...
	{
		$model = &JModel::getInstance('listcat','JCollectionModel');
	...
	function _buildQuery()
	{
		...
		$query = " SELECT c.*, u.name AS uname, u.username, g.name AS groupname  "
		."\n FROM #__"._JC_DB."_listcat AS c "
	...
	function _buildContentWhere()
	{
		...
		$where = array();
		if($search) {
			$where[] = '( LOWER( c.name ) LIKE '.$this->_db->Quote( '%'.$this->_db->getEscaped( $search, true ).'%', false ).' )';
		}
		if($filter_access != -1) {
			$where[] = '( c.access = '.(int)$filter_access.' )';
		}
		if($filter_approved != -1) {
			$where[] = '( c.approved = '.(int)$filter_approved.' )';
		}
		if($filter_state == "P") {
			$where[] = '( c.published > 0 )';
