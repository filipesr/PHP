...
class Customer {
	...
	private $customer_group_min_order;
	...
	private $address_id;
  	...
  	public function __construct($registry) {
		...
		if (isset($this->session->data['customer_id'])) { 
			$customer_query = $this->db->query("SELECT c.*, g.min_order, d.name FROM " . DB_PREFIX . "customer c inner join customer_group g on c.customer_group_id = g.customer_group_id inner join customer_group_description d on c.customer_group_id = d.customer_group_id WHERE customer_id = '" . (int)$this->session->data['customer_id'] . "' AND status = '1'");
			...
			if ($customer_query->num_rows) {
				...
				$this->customer_group_min_order = $customer_query->row['min_order'];
	...
	public function logout() {
  	...
  	public function getCustomerGroupMinOrder() {
		return $this->customer_group_min_order;	
  	}
