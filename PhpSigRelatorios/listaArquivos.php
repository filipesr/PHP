...
if (!$usuario->isLogged()) {
    if (!$usuario->login($_POST['login'], $_POST['pass'])) {
        header("Location: " . HTTP_SERVER . "index.php?msgErro=" . urlencode("Senha ou usuário inválido!"));
        exit;
    } else {
                    ...
                    <div class="selClienteCategoria">
                        ...
                        Categoria: <select>
                            ...
                            <?php foreach ($usuario->getCategorias() as $categoria) { ?>
                                <option style="background-color: #<?php echo $categoria["cor"]; ?>" value="<?php echo $categoria["idcategoria"]; ?>"><?php echo $categoria["nome"]; ?></option>
                            <?php } ?>
                        </select>
                    </div>
